...
export function Cube({
  ...
  useFrame((frameState, delta) => {
    ...
    state.currentAnimSpeed = lerp(
      state.currentAnimSpeed,
      colorConfig.animationSpeed,
      transitionSpeed
    );
    state.currentBreathSpeed = lerp(
      state.currentBreathSpeed,
      colorConfig.breathingSpeed,
      transitionSpeed
    );
    state.currentGlowIntensity = lerp(
      state.currentGlowIntensity,
      colorConfig.glowIntensity,
      transitionSpeed
    );
    state.currentMouseFollowSpeed = lerp(
      state.currentMouseFollowSpeed,
      colorConfig.mouseFollowSpeed,
      transitionSpeed
    );
    ...
    if (animationState === "listening") {
      ...
      const nodSpeed = 1.8;
      const nodAngle =
        Math.sin(state.listeningIntensity * nodSpeed) * ((10 * Math.PI) / 180);
    ...
    if (animationState === "thinking") {
      ...
      state.targetStateRotation.z =
        Math.sin(state.thinkingTime * 0.3) * ((3 * Math.PI) / 180);
    } else {
    ...
    if (animationState === "speaking") {
      ...
      const speakNodAngle =
        Math.sin(state.speakingTime * speakNodSpeed) * ((7 * Math.PI) / 180);
      state.targetStateRotation.x = speakNodAngle;
      state.targetStateRotation.z =
        Math.sin(state.speakingTime * 1.5) * ((2 * Math.PI) / 180);
    } else {
    ...
    state.currentStateRotation.x = lerp(
      state.currentStateRotation.x,
      state.targetStateRotation.x,
      delta * rotationTransitionSpeed
    );
    state.currentStateRotation.y = lerp(
      state.currentStateRotation.y,
      state.targetStateRotation.y,
      delta * rotationTransitionSpeed
    );
    state.currentStateRotation.z = lerp(
      state.currentStateRotation.z,
      state.targetStateRotation.z,
      delta * rotationTransitionSpeed
    );
    ...
    state.currentMouseRotation.x +=
      (state.targetMouseRotation.x - state.currentMouseRotation.x) *
      state.currentMouseFollowSpeed;
    state.currentMouseRotation.y +=
      (state.targetMouseRotation.y - state.currentMouseRotation.y) *
      state.currentMouseFollowSpeed;

    // === APPLY ROTATION ===
    // meshRef.current.rotation.x =
    //   state.currentMouseRotation.x + idleSwayX + state.currentStateRotation.x;
    // meshRef.current.rotation.y =
    //   state.currentMouseRotation.y + idleSwayY + state.currentStateRotation.y;
    ...
    // meshRef.current.rotation.z = state.currentStateRotation.z + Math.PI /6;
    // front‑top corner pointing toward camera
   
    const baseTiltX = Math.PI / 5.4;
    const baseTurnY = Math.PI / 5;
    const baseRollZ = 0;
    ...
    meshRef.current.rotation.z = baseRollZ + state.currentStateRotation.z; // or + small value if you want a slight roll
    ...
    // === BREATHING / GLOW ===
    let breathingIntensity =
      state.currentGlowIntensity + Math.sin(state.breathingPhase) * 0.15;
