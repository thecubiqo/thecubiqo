...
export const V2_CAPABILITIES: V2Capability[] = [
  ...
  {
    actionType: 'shopify_read',
    toolName: 'shopify_read',
    label: 'Shopify read API',
    category: 'Business/POD',
    status: 'active',
    approvalRequired: false,
    approvalRequestable: false,
    endpoint: '/api/connectors/status',
    summary: 'Reads Shopify products, orders, and inventory through server-side Admin API tokens only.',
    requirements: ['signed-in user', 'encrypted Shopify token', 'no token in response'],
    externalAction: true
  },
  {
    actionType: 'shopify_write',
    toolName: 'shopify_write',
    label: 'Shopify write API',
    category: 'Business/POD',
    status: 'active',
    approvalRequired: true,
    approvalRequestable: false,
    endpoint: '/api/actions/pod-product',
    summary: 'Creates or updates Shopify products only through approved server-side actions. Billing, payment, and checkout paths are hard-blocked.',
    requirements: ['signed-in user', 'encrypted Shopify token', 'approval card', 'audit log'],
    externalAction: true
  },
  {
    actionType: 'printify_read',
    toolName: 'printify_read',
    label: 'Printify read API',
    category: 'Business/POD',
    status: 'active',
    approvalRequired: false,
    approvalRequestable: false,
    endpoint: '/api/connectors/status',
    summary: 'Reads Printify providers, products, and orders through a server-side encrypted API key.',
    requirements: ['signed-in user', 'encrypted Printify API key', 'no key in response'],
    externalAction: true
  },
  {
    actionType: 'printify_write',
    toolName: 'printify_write',
    label: 'Printify write API',
    category: 'Business/POD',
    status: 'active',
    approvalRequired: true,
    approvalRequestable: false,
    endpoint: '/api/actions/pod-product',
    summary: 'Creates Printify products and submits approved print operations through server-side API calls.',
    requirements: ['signed-in user', 'encrypted Printify API key', 'approval card', 'audit log'],
    externalAction: true
  },
  {
    actionType: 'shopify_billing',
    toolName: 'shopify_billing',
    ...
    approvalRequestable: false,
    endpoint: null,
    summary: 'Billing, payment, and checkout APIs are hard-blocked in V2.',
    requirements: ['not available in V2'],
    externalAction: true
  },
  {
    ...
    approvalRequestable: true,
    endpoint: '/api/actions/pod-product',
    summary: 'Creates a linked Printify and Shopify draft product after a full approval card.',
    requirements: ['approved product preview', 'Shopify connector', 'Printify connector', 'media assets', 'audit log'],
    externalAction: true
  },
  {
    actionType: 'pod_product_publish',
    toolName: 'pod_product_publish',
    label: 'Publish POD product',
    category: 'Business/POD',
    status: 'active',
    approvalRequired: true,
    approvalRequestable: true,
    endpoint: '/api/actions/pod-product/publish',
    summary: 'Publishes an existing draft POD product only after a separate user confirmation approval.',
    requirements: ['approved publish preview', 'existing POD product', 'audit log'],
    externalAction: true
  },
  {
