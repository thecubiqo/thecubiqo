...
export async function GET(request: NextRequest) {
  try {
    ...
    if (catalogError) {
      ...
      // If table doesn't exist yet, return empty catalog
      if (catalogError.code === 'PGRST116' || catalogError.message?.includes('does not exist')) {
        console.warn('features_catalog table not found - returning empty catalog')
        return NextResponse.json({
          features: [],
          categories: [],
          active_design: 'design.plasma_wave',
          error: 'Database migration pending. Please run: supabase db push'
        })
      }
