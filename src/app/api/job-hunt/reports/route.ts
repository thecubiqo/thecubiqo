...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        const emailResponse = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: process.env.SELF_HEAL_EMAIL_FROM || 'CubiQo Job Hunt <noreply@cubiqo.ai>',
            to: [userProfile.email],
            subject: reportSubject,
            html: `
              <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                <div style="text-align: center; margin-bottom: 24px;">
                  <div style="display: inline-block; width: 40px; height: 40px; background: linear-gradient(135deg, #f97316, #ef4444); border-radius: 12px; line-height: 40px; color: white; font-weight: bold; font-size: 18px;">Q</div>
                  <h1 style="margin: 12px 0 0; font-size: 20px; color: #333;">CubiQo Job Hunt</h1>
                </div>
                ${reportContent}
                <hr style="border: none; border-top: 1px solid #eee; margin: 24px 0;" />
                <p style="font-size: 12px; color: #999; text-align: center;">
                  This report was generated by CubiQo Job Hunt Mode.<br/>
                  <a href="https://cubiqo.ai/job-hunt" style="color: #f97316;">View Dashboard</a>
                </p>
              </div>
            `,
          }),
        ...
        if (emailResponse.ok) {
          emailSent = true
          console.log(`[Job Hunt] Email sent to ${userProfile.email}: ${reportSubject}`)
        } else {
          const errData = await emailResponse.json()
          console.error('[Job Hunt] Resend error:', errData)
        }
      } catch (emailErr) {
        console.error('[Job Hunt] Failed to send email:', emailErr)
      }
    } else {
      console.warn('[Job Hunt] RESEND_API_KEY not set — skipping email delivery')
    }
    ...
    await supabase.from('job_hunt_activities').insert({
      ...
      description: `Report ${emailSent ? 'sent' : 'generated'}: ${reportSubject}`,
      details: { report_type: body.report_type, email_sent: emailSent },
    })
