...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!vercelToken) {
      // Graceful degradation — return queued status with a note
      ...
      return NextResponse.json({
        ...
        deployment: {
          ...
          status: 'pending',
          url: null,
          message: 'VERCEL_TOKEN not configured — deployment cannot be triggered. Set the VERCEL_TOKEN environment variable to enable deployments.'
        }
      }, { status: 202 });
    }

    // Fetch project details from database to get the repository info
...
export async function GET(request: NextRequest) {
  try {
    ...
    const vercelResponse = await fetch(`${VERCEL_API}/v13/deployments/${deploymentId}`, {
      headers: {
        Authorization: `Bearer ${vercelToken}`,
      },
    });
