 ...
 *
 * POST – Executes a sandboxed command inside a workspace directory.
 * GET  – Returns 501 because Next.js cannot upgrade to WebSocket.
 */
...
export async function GET(request: NextRequest) {
  ...
  return new Response(
    JSON.stringify({
      ...
      documentation:
        'See /docs/EMERGENT_REQUIREMENTS_EXTRACTED.md for Runner System requirements',
    }),
    {
      status: 501,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ── 5. Error handling ──────────────────────────────────────────────
    // child_process.exec rejects with an error that carries extra fields
