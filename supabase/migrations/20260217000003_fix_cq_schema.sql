-- CQ-to-CQ Messaging System Database Schema - Fix for Staging
-- Applies the correct schema required by the frontend application
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_friend_requests_unique ON cq_friend_requests(from_user_id, to_user_id) 
...
CREATE INDEX IF NOT EXISTS idx_contacts_pinned ON cq_contacts(user_id, is_pinned) WHERE is_pinned = true;
...
CREATE INDEX IF NOT EXISTS idx_messages_status ON cq_messages(status);
CREATE INDEX IF NOT EXISTS idx_messages_unread ON cq_messages(to_user_id, read_at) WHERE read_at IS NULL;
...
-- Indexes for calls
CREATE INDEX IF NOT EXISTS idx_calls_conversation ON cq_calls(conversation_id, started_at DESC);
CREATE INDEX IF NOT EXISTS idx_calls_initiator ON cq_calls(initiator_id);
CREATE INDEX IF NOT EXISTS idx_calls_recipient ON cq_calls(recipient_id);
CREATE INDEX IF NOT EXISTS idx_calls_status ON cq_calls(status);

-- Screen share sessions
CREATE TABLE IF NOT EXISTS cq_screen_shares (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_screen_shares_call ON cq_screen_shares(call_id);
...
CREATE INDEX IF NOT EXISTS idx_notifications_user ON cq_notifications(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_notifications_unread ON cq_notifications(user_id, read) WHERE read = false;

-- Privacy settings table
CREATE TABLE IF NOT EXISTS cq_privacy_settings (
...
CREATE TABLE IF NOT EXISTS cq_voice_synthesis (
  user_id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS cq_premium_status (
  user_id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
...
DROP TRIGGER IF EXISTS trigger_update_conversation_on_message ON cq_messages;
...
DROP TRIGGER IF EXISTS trigger_reset_unread_on_read ON cq_messages;
...
-- Note: We drop existing policies if they exist to avoid errors on duplicate creation, 
-- or we can use DO blocks. For simplicity in this migration, we'll assume they might not exist 
-- or strictly create them. To be safe, we can drop them first.
...
DROP POLICY IF EXISTS privacy_settings_policy ON cq_privacy_settings;
CREATE POLICY privacy_settings_policy ON cq_privacy_settings FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS voice_synthesis_policy ON cq_voice_synthesis;
CREATE POLICY voice_synthesis_policy ON cq_voice_synthesis FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS premium_status_policy ON cq_premium_status;
CREATE POLICY premium_status_policy ON cq_premium_status FOR ALL USING (auth.uid() = user_id);
